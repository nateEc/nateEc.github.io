...
const scenes: TourScene[] = [
  {
    ...
    title: { en: 'Agent workbench', zh: 'Agent 工作台' },
    detail: { en: 'A real packaged Electron window, mounted to the local t3code workbench after HiPilot authentication.', zh: '真实打包后的 Electron 窗口，在 HiPilot 本地认证后挂载 t3code 工作台。' },
    callout: { en: 'One surface for model, mode, workspace, and thread state.', zh: '模型、运行模式、工作区与线程状态集中在同一界面。' },
    position: '30% 72%',
  },
  {
    id: 'terminal',
    ...
    detail: { en: 'The app opens a real PTY inside the selected workspace; this capture contains only the local smoke marker.', zh: '应用会在所选工作区中打开真实 PTY；这张截图只保留了本地 smoke marker。' },
    callout: { en: 'Command evidence stays attached to the task context.', zh: '命令证据与当前任务上下文保持在一起。' },
    position: '55% 78%',
  },
  {
    id: 'panels',
  ...
  },
  {
    ...
    title: { en: 'Desktop preferences', zh: '桌面端设置' },
    detail: { en: 'The shipped app exposes language, theme, safety defaults, runtime checks, and redacted diagnostics.', zh: '已交付应用公开语言、主题、安全默认值、运行时检查和脱敏诊断设置。' },
    callout: { en: 'Product policy is visible and configurable—not hidden in env files.', zh: '产品策略可见、可配置，而不是只藏在环境变量里。' },
    position: '82% 38%',
  },
]
...
const copy = computed(() => props.language === 'zh'
  ? {
      ...
      scene: '场景',
      verified: '本地验证',
      verifiedValue: 'API · DB · Redis healthy',
      auth: '认证',
      authValue: '本地 smoke 会话',
      privacy: '展示数据',
      privacyValue: '历史侧栏已裁除',
  ...
  : {
      ...
      verifiedValue: 'API · DB · Redis healthy',
      auth: 'Authentication',
      authValue: 'Local smoke session',
      privacy: 'Display data',
      privacyValue: 'History sidebar cropped',
