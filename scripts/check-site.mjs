...
assert(source.includes('/apple-touch-icon.png?v=20260907') && source.includes('wordmark__mark'), 'navigation wordmark uses the portrait icon')
...
assert(!read('src/components/Navigation.vue').includes("key: 'worklog'"), 'weekly worklog is removed from navigation')
...
assert(techNewsPayload.schemaVersion === 2, 'Tech Signal payload declares its bilingual schema version')
...
assert(Array.isArray(techNewsPayload.sections) && techNewsPayload.sections.length === 2, 'Tech Signal contains exactly two sources')
const techNewsSourceNames = new Set(techNewsPayload.sections.map((section) => section.name))
assert(['Hacker News', 'TechCrunch'].every((name) => techNewsSourceNames.has(name)), 'Tech Signal contains every required source')
assert(techNewsPayload.sections.every((section) => Array.isArray(section.items) && section.items.length === 8), 'Every Tech Signal source contains exactly eight items')
...
assert(!sitemap.includes('/worklog'), 'sitemap excludes the retired weekly worklog')
