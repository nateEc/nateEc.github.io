...
assert(['AI资讯', 'Hacker News', 'TechCrunch'].every((name) => techNewsSourceNames.has(name)), 'Tech Signal contains every required source')
assert(techNewsPayload.sections.every((section) => Array.isArray(section.items) && section.items.length > 0), 'Every Tech Signal source contains displayable items')
