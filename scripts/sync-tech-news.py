...
PROJECT_ROOT = Path('/Users/nathanshan/Desktop/nateEc.github copy.io')
...
def _build_ai_section(data: dict[str, Any], source_name: str) -> dict[str, Any]:
    ...
    for item in raw_items[:4]:
        ...
        reasons = item.get('why_trending') or []
        items.append({
            'title': title,
            'summary': summary,
            'url': url,
            ...
            'published': _normalize_text(item.get('published', ''), 24),
        })
    return {'name': source_name, 'source': source_name, 'items': items}
...
def _build_hn_sections(data: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for source in data.get('sources', []):
        ...
        source_page = source.get('source_page', source.get('feed_url', name))
